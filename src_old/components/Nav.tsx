...
function NavLanguage({
  ...
  return (
    <div className={`language-selector language-selector--${variant}`}>
      <button type="button" className="language-button" onClick={onToggle}>
        <Globe size={20} />
      ...
      {showLanguageMenu && (
        <div className="language-menu">
          ...
          <button type="button" onClick={() => onChangeLanguage("pt")}>
            Português
          </button>
        </div>
      )}
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      languageSelectors.forEach((el) => {
        if (el.contains(event.target as Node)) insideLanguage = true;
      });
      if (!insideLanguage) {
